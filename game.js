...
const HIDDEN_ENDING_STORAGE_KEY = "littleRedHiddenEndingUnlocked";
const HIDDEN_ENDING_TITLE = "Ending ???";
const endingCatalog = [
...
let hiddenEndingUnlocked = loadHiddenEndingUnlocked();
...
function loadCollectedEndings() {
	try {
		...
		if (Array.isArray(parsed)) {
			return parsed.filter((ending) => endingCatalog.includes(ending) || ending === HIDDEN_ENDING_TITLE);
		}
	} catch (error) {
...
function loadHiddenEndingUnlocked() {
	try {
		return localStorage.getItem(HIDDEN_ENDING_STORAGE_KEY) === "true";
	} catch (error) {
		console.error("Failed to load hidden ending state", error);
	}
...
function saveHiddenEndingUnlocked() {
	localStorage.setItem(HIDDEN_ENDING_STORAGE_KEY, hiddenEndingUnlocked ? "true" : "false");
}
...
function renderEndingCollection() {
	const visibleCatalog = hiddenEndingUnlocked ? [...endingCatalog, HIDDEN_ENDING_TITLE] : endingCatalog;
	const unlockedCount = collectedEndings.filter((ending) => visibleCatalog.includes(ending)).length;
	endingCollectionCount.textContent = `${unlockedCount} / ${visibleCatalog.length} unlocked`;
	collectionBtn.textContent = `Ending Collection (${unlockedCount}/${visibleCatalog.length})`;

	endingCollectionList.innerHTML = "";
	visibleCatalog.forEach((title) => {
...
// Kitten easter egg logic
const KITTY_CLICKS_KEY = "littleRedKittyClicks";
let kittyClicks = 0;

function loadKittyClicks() {
	try {
		const stored = localStorage.getItem(KITTY_CLICKS_KEY);
		if (stored) {
			return parseInt(stored, 10) || 0;
		}
	} catch (error) {
		console.error("Failed to load kitty clicks", error);
	}
	return 0;
}

function saveKittyClicks() {
	localStorage.setItem(KITTY_CLICKS_KEY, String(kittyClicks));
}
...
// Kitty button event listeners
const kittyBtn = document.getElementById("kittyBtn");
const kittyImg = document.getElementById("kittyImg");

if (kittyBtn && kittyImg) {
	kittyClicks = loadKittyClicks();
	
	kittyBtn.addEventListener("mouseenter", () => {
		kittyImg.src = "pic/2.PNG";
	});
	
	kittyBtn.addEventListener("mouseleave", () => {
		kittyImg.src = "pic/1.PNG";
	});
	
	kittyBtn.addEventListener("click", (e) => {
		e.preventDefault();
		kittyImg.src = "pic/3.PNG";
		kittyClicks++;
		saveKittyClicks();
		
		// Check if hidden ending is unlocked
		if (kittyClicks >= 100) {
			sessionStorage.setItem("endingData", JSON.stringify({
				title: HIDDEN_ENDING_TITLE,
				text: "...Something feels strange.\nWhy is everything covered in little cat faces?\nWait.\nAm I the real little red hooding...\nor just a tiny cat inside someone's game?",
				image: "pic/ending5.png",
				isHidden: true
			}));
			hiddenEndingUnlocked = true;
			saveHiddenEndingUnlocked();
			window.location.href = "ending.html";
			return;
		}
		
		// Change back to normal after delay
		setTimeout(() => {
			kittyImg.src = "pic/1.PNG";
		}, 150);
	});
